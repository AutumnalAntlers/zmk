...
struct character_map_config {
    ...
    const char *unicode_behavior_dev;
...
static int codepoint_to_binding(const struct device *dev, uint32_t codepoint,
    ...
    if (config->unicode_behavior_dev) {
        *binding = (struct zmk_behavior_binding){.behavior_dev = config->unicode_behavior_dev,
                                                 .param1 = codepoint};
        return 0;
    }
...
#define CHARMAP_INST(n)                                                                            \
    ...
    static const struct character_map_config character_map_config_##n = {                          \
        .behavior_dev = DT_PROP(DT_INST_PROP(n, behavior), label),                                 \
        .unicode_behavior_dev =                                                                    \
            COND_CODE_1(DT_INST_NODE_HAS_PROP(n, unicode_behavior),                                \
                        (DT_PROP(DT_INST_PROP(n, unicode_behavior), label)), (NULL)),              \
