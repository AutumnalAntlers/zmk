...
/**
 * Assert at compile time that a zmk,character_map chosen node is set.
 */
#define ZMK_BUILD_ASSERT_CHARACTER_MAP_CHOSEN()                                                    \
    BUILD_ASSERT(                                                                                  \
        DT_HAS_CHOSEN(zmk_character_map),                                                          \
        "A zmk,character_map chosen node must be set. See "                                        \
        "https://zmk.dev/docs/behaviors/send-string#character-maps for more information.");

/**
 * Get a struct zmk_send_string_config which uses the zmk,character_map chosen node
 * and Kconfig options for timing.
 *
 * Use ZMK_BUILD_ASSERT_CHARACTER_MAP_CHOSEN() before using this macro to provide
 * a nice error message if a character map hasn't been set.
 */
 ...
 * zmk,character_map chosen node is set.
 */
#define ZMK_BUILD_ASSERT_CHARACTER_MAP_DT_INST_PROP(n)                                             \
    BUILD_ASSERT(DT_INST_NODE_HAS_PROP(n, charmap) || DT_HAS_CHOSEN(zmk_character_map),            \
                 "Node " DT_NODE_PATH(DT_DRV_INST(                                                 \
                     n)) " requires a charmap property or a zmk,character_map chosen node. "       \
 ...
 *     type: int
 *
 * Use ZMK_BUILD_ASSERT_CHARACTER_MAP_DT_INST_PROP(n) before using this macro to
 * provide a nice error message if a character map hasn't been set.
 */
#define ZMK_SEND_STRING_CONFIG_DT_INST_PROP(n)                                                     \
    ((struct zmk_send_string_config){                                                              \
        .character_map = DEVICE_DT_GET(DT_INST_PROP_OR(n, charmap, DT_CHOSEN(zmk_character_map))), \
